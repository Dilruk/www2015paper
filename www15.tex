...
\maketitle
\begin{abstract}
Much of the existing research into event detection in social media relaxes the problem by assuming a specific language, targeting fixed event types, and tracking bursts of seed keywords relevant to the target domain.
While such systems demonstrate acceptable performance, they suffer from a number of important drawbacks, most notable of which are requiring a priori knowledge of relevant keywords and brittleness in adapting to new languages or events. 
This paper extends such approaches by introducing a technique for modeling temporal patterns exhibited by keywords experiencing bursts and by building a classifier that can identify these bursting keywords from Twitter's unfiltered 1\% public stream.
By focusing only on these temporal characteristics in burst identification, our approach achieves a measure of language agnosticism current techniques lack.
...
\category{H.3.3}{Inform. Search and Retrieval}{Information Filtering}
...
While such social media-based systems are able to detect noteworthy events rapidly, they often come with restrictions needed to relax the event-detection task. 
Such restrictions might include pre-specifying classes of interesting events, limiting target languages, setting event-centric queries, and preprocessing/ filtering data through expensive language models.
...
To ground this exploration, we start by modeling large sporting competitions since such events are both highly followed and occur regularly but also include unpredictable patterns of sub-events around points scored, fouls committed, and other occurrences of high interest.
Experiments on several such sporting competitions illustrate how a language-agnostic, burst-centric technique performs in comparison to a baseline keyword-centric method while simultaneously providing additional insight across languages and without text normalization or filtering.
...
In each competition, we extract the times of four basic types of events (beginning of the game, end of the game, scores, and penalties) from existing blog posts, news articles, and social media to construct game timelines.
