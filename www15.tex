...
\maketitle
\begin{abstract}
Existing research into event detection in social media often targets specific events by tracking relevant seed keywords, a method that suffers from significant drawbacks like requiring a priori knowledge of relevant tokens and difficulties in adapting to new domains or languages. 
This paper extends such approaches by introducing a general technique for identifying moments of interest, which often correspond to events, by modeling temporal patterns around bursts and by building a classifier that can identify bursting keywords from Twitter's public sample stream.
By focusing only on these temporal characteristics in identifying bursts, our approach achieves a measure of language agnosticism and general flexibility current techniques lack.
...
Such systems generally require a human expert to generate a set of seed tokens to track across social media, and while these systems are able to detect noteworthy events rapidly, they often come with restrictions needed to make the event-detection task tractable. 
...
Most importantly, however, the critical weakness here is that such approaches preclude any  \emph{discovery} of interesting events; since there is no flexibility in identifying new tokens automatically, many interesting moments related to the target event might be lost because they may not mention the pre-specified token, which could occur for any number of reasons (different language, colloquialisms, or particular names of people or places).

Forgoing such simplifying restrictions and processing unfiltered social media streams is a more difficult task given the volume of noise in such sources, but the flexibility to discover surprising or unexpected moments across multiple languages/dialects has many practical advantages currently unsatisfied by existing technology.
Given the ``unreasonable effectiveness of data'' and the shear volume of social media content  (hundreds of thousands of comments, statuses, and photos generated per minute on Facebook alone as of 2012 \cite{Pring2012}), temporal patterns alone might alleviate these dependencies.

To explore these questions, we propose a general method for identifying interesting moments in social media streams by learning patterns of increased token usage, or bursts.
...
\item By comparing our method to an existing event detection baseline system, we demonstrate our generalization of events to interesting moments is still highly performant.
...
Though we are focusing on the slightly different problem of identifying ``interesting moments'' in social media streams, we still build on much of the same work as classical event detection, which has fascinated researchers for over twenty years, with new methods, breakthroughs, and technologies emerging every few years.
...
Examples of such moments include a point scored in a game, a single explosion during some terrorist attack, or a particular tremor in an earthquake.
