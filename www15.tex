...
\maketitle
\begin{abstract}
Existing event detection approaches in social media often targets specific events by tracking human-generated seed keywords related to the event. \red{in the first sentence, you get yourself into the whole what's an event vs. sub-event debate... this is confusing.}
...
This paper extends \red{you don't extend, you propose something fundamentally different.} these approaches by introducing a general technique for identifying compelling moments without seed keywords by modeling temporal patterns around bursts and by building a classifier that can identify bursting tokens from Twitter's public sample stream.
Since many events are also interesting moments, we are also able to subsume some event detection tasks and achieve a measure of language agnosticism and general flexibility current techniques lack. \red{this is the key point, lead with this in the abstract.}
...
Results from this comparison show our method demonstrates similar performance in this sports detection domain without the need to pre-specify any event-related information. \red{again, what's the event?}
